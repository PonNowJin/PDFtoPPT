...
OUTPUT_ANNOTATED_DIR = (
    "temp_output/annotated_results"  # 用於存標註好的結果（要將debug開啟才有用）
)
...
def paper_to_slide(pdf_path, output_pptx_path, theme_pptx_path):
    try:
        ...
        print("使用 YOLO 偵測圖區...")
        metadata = detect_and_annotate_images(
            image_paths, OUTPUT_ANNOTATED_DIR, crop_img_dir=CROP_IMGS_DIR, debug=False
        )
        ...
        if theme_pptx_path:
            json_to_pptx(
                ...
                output_pptx_path,
                output_pptx_path,
                image_dir=CROP_IMGS_DIR,
                template=theme_pptx_path,
                premium=True,
            )
        else:
            json_to_pptx(
